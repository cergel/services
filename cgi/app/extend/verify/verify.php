...
namespace sdkService\extend\verify;
/**
 * 1.输出验证码图片
 * 2.将验证码存储
 * Class verify
 * @package app\extend\verify
 */
class verify
{
    protected $config = array(
        'seKey' => 'wepiao.com',   // 验证码加密密钥
        'codeSet' => '234678abcdefhkmnprtuvwxyz',             // 验证码字符集合
        #'codeSet' => '0123456789',
    ...
    //生成验证码图片
    public function createCodePic($tmpFilePath)
    {
        // 图片宽(px)
        ...
        $this->_color = imagecolorallocate($this->_image, 3, 175, 14);
        // 验证码使用随机字体
        ...
        $this->fontttf='';//这里要赋值个'',不然在下面的循环中会出错

        if(!is_dir(dirname($tmpFilePath))){
            $mkdirResult = mkdir(dirname($tmpFilePath),755,1);
            if(!$mkdirResult){
                throw new \Exception("make dir error :".dirname($tmpFilePath));
            }
        }
        imagepng($this->_image,$tmpFilePath);//这里的值是传进来的
        imagedestroy($this->_image);

        $strPic=file_get_contents($tmpFilePath);
        $return[strtoupper(implode('', $code))]=$strPic;
        return $return;
    }
